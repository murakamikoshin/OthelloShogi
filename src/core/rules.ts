 ...
 *
 * 先手だけ歩が1枚多い。「先に陣形を崩した側が狙われる」という後手有利が
 * はっきり出たため、先手に持ち駒1枚ぶんのハンデ差を付けている（囲碁のコミに相当）。
 *
 * AI 同士の対局で測った先手勝率:
 *   ハンデ無し  40%（60局）
 *   歩1枚        59%（100局）
 *
 * 歩1枚はやや効きすぎだが、駒は1枚単位でしか動かせない。
 * 将棋やチェスと同じく「先手がわずかに有利」の側に倒してある。
 */
...
/**
 * 盤の端を挟みの終端（アンカー）として認めるか。
 *
 * true だと「相手の駒が続いた先が盤の外」でも挟みが成立する。
 * 逃げ場を失った兵が降伏する、という見立て。
 *
 * AI 同士40局ずつの比較で、これがいちばん効いた:
 *
 *   　　　　　　　　　　3連鎖以上が出た対局   1手あたりの反転枚数
 *   　無し　　　　　　　       15%                  0.76
 *   　有り　　　　　　　       85%                  2.51
 *
 * 端に追い込む動きに意味が出て、連鎖がゲームの主役になる。
 */
export const WALL_ANCHORS = true;

/**
 * 打った駒自身が成るのに必要な反転枚数。0 なら成らない（打つときは常に不成）。
 *
 * 大量に寝返らせた手柄で打った駒も出世する、という報酬。
 * 連鎖を狙う動機を強くするために入れている。
 */
export const PROMOTE_DROP_AT = 0;
...
/**
 * この手数で決着しなければ、盤上の駒数が多い方の勝ちにする。0 なら無制限。
 *
 * これが無いと決着が玉取りばかりになり、オセロ側の陣取りが勝敗に効かない。
 * 入れると「玉を狙う」「駒数で勝つ」の二択が常にあることになる。
 */
export const PLY_LIMIT = 0;
